...
/*
         ...
         // addWeighted(frame, 0.0, img_line, 1.0, 0.0, img_result);
           // addWeighted(frame, 0.0, img_masked, 1.0, 0.0, img_result);
            ...
            vector<uchar> encode1;
            vector<uchar> encode2;
            vector<int> encode_param;
            
            encode_param.push_back(CV_IMWRITE_JPEG_QUALITY);
            encode_param.push_back(20);
            
            imencode(".jpg", img_line, encode1, encode_param);
            imencode(".jpg", img_masked, encode2, encode_param);
            Mat decode1 = imdecode(encode1, 1);
            Mat decode2 = imdecode(encode2, 1);
            imshow("decode1", decode1);
            imshow("decode2", decode2);
            //imshow("img_mask", img_mask);
            // Convert encoded image to ROS std_msgs format
            std_msgs::UInt8MultiArray msgArray;
            msgArray.data.clear();
            msgArray.data.resize(encode1.size());
            copy(encode1.begin(), encode1.end(), msgArray.data.begin());


            msgArray.data.resize(encode2.size());
            copy(encode2.begin(), encode2.end(), msgArray.data.begin());
            // Publish msg
